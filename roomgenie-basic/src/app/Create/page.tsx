...
const IMGS = {
  heroBefore:  'https://images.unsplash.com/photo-1600210492493-0946911123ea?w=1400&q=85&auto=format&fit=crop',
...
const btn = (highlight: boolean) => ({
  padding: highlight ? '16px 38px' : '16px 30px',
  borderRadius: 14,
  fontSize: 15,
  fontWeight: 800,
  cursor: 'pointer',
  border: highlight ? 'none' : '1px solid rgba(255,255,255,0.13)',
  background: highlight ? 'linear-gradient(135deg,#4f7cff,#8b5cf6)' : 'rgba(255,255,255,0.06)',
  color: 'white',
  boxShadow: highlight ? '0 8px 28px rgba(79,124,255,0.4)' : 'none',
...
const h2style = {
  fontSize: 'clamp(28px,4vw,50px)' as string,
  fontWeight: 900,
  letterSpacing: '-1.5px',
  color: 'white',
  lineHeight: 1.1,
...
export default function Home() {
  return (
    <main style={{ background: '#06080f', color: 'white', minHeight: '100vh' }}>
      ...
      {/* ── NAV ── */}
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 200, background: 'rgba(6,8,15,0.75)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 28px', height: 66, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <button style={{ background: 'none', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.6)', fontSize: 13, fontWeight: 600, padding: '9px 20px', borderRadius: 10, cursor: 'pointer' }}>Sign In</button>
            <a href="/create" style={{ ...btn(true), fontSize: 14, padding: '10px 22px', textDecoration: 'none', display: 'inline-block' }}>Start Free →</a>
          </div>
        </div>
      </nav>
      ...
      <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '130px 24px 60px', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
          <img src={IMGS.heroBefore} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.07, filter: 'saturate(0.4)' }} />
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom,#06080f 0%,rgba(6,8,15,0.3) 40%,rgba(6,8,15,0.9) 75%,#06080f 100%)' }} />
        </div>
        <div style={{ position: 'absolute', top: '30%', left: '50%', transform: 'translateX(-50%)', width: 900, height: 600, background: 'radial-gradient(ellipse,rgba(79,124,255,0.14) 0%,transparent 65%)', pointerEvents: 'none', borderRadius: '50%', zIndex: 1 }} />

        {/* Badge */}
        <div style={{ ...label, position: 'relative', zIndex: 2, display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 28 }}>
          <span style={{ color: '#fbbf24' }}>⚡</span> Powered by GPT-Image-1 · Renders in 10 seconds
        </div>

        {/* Headline */}
        <h1 style={{ position: 'relative', zIndex: 2, fontSize: 'clamp(40px,6.5vw,80px)', fontWeight: 900, lineHeight: 1.04, letterSpacing: '-2.5px', maxWidth: 900, marginBottom: 22 }}>
        ...
        <p style={{ position: 'relative', zIndex: 2, fontSize: 18, color: 'rgba(255,255,255,0.45)', maxWidth: 520, lineHeight: 1.75, marginBottom: 36 }}>
          Upload your room photo, choose a style, and get a photorealistic redesign in seconds. No skills. No software. Just magic.
        </p>

        <div style={{ position: 'relative', zIndex: 2, display: 'flex', gap: 14, flexWrap: 'wrap', justifyContent: 'center', marginBottom: 44 }}>
          <a href="/create" style={{ ...btn(true), fontSize: 16, textDecoration: 'none' }}
            onMouseOver={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 14px 40px rgba(79,124,255,0.55)' }}
          ...
          <button style={btn(false)}
            ...
            onMouseOut={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.06)'; e.currentTarget.style.color = 'white' }}>
            ▶ &nbsp;Watch Demo
          </button>
        </div>

        <div style={{ position: 'relative', zIndex: 2, display: 'flex', gap: 24, flexWrap: 'wrap', justifyContent: 'center', fontSize: 13, color: 'rgba(255,255,255,0.3)', marginBottom: 64 }}>
          {['⭐ 4.9/5 from 2,000+ users', '🔒 No credit card', '🎁 3 free renders/month', '⚡ 10,000+ rooms designed'].map(t => <span key={t}>{t}</span>)}
        </div>

        {/* BEFORE / AFTER */}
        <div style={{ position: 'relative', zIndex: 2, width: '100%', maxWidth: 1100 }}>
          <div style={{ position: 'absolute', inset: -2, borderRadius: 28, background: 'linear-gradient(135deg,rgba(79,124,255,0.4),rgba(139,92,246,0.25))', filter: 'blur(2px)', zIndex: 0 }} />
          <div style={{ position: 'relative', zIndex: 1, borderRadius: 24, overflow: 'hidden', border: '1px solid rgba(79,124,255,0.2)', boxShadow: '0 40px 100px rgba(0,0,0,0.7)' }}>
            {/* Browser bar */}
            <div style={{ background: '#0d1120', borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ flex: 1, maxWidth: 300, margin: '0 auto', background: 'rgba(255,255,255,0.05)', borderRadius: 7, padding: '4px 14px', fontSize: 12, color: 'rgba(255,255,255,0.25)', textAlign: 'center' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', position: 'relative' }}>
              <div style={{ position: 'relative', overflow: 'hidden', aspectRatio: '16/9' }}>
                <img src={IMGS.heroBefore} alt="Before" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'saturate(0.35) brightness(0.55)' }} />
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to right,transparent 60%,#0d1120 100%)' }} />
                <div style={{ position: 'absolute', top: 14, left: 14, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(8px)', borderRadius: 8, padding: '5px 13px', fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,0.5)', border: '1px solid rgba(255,255,255,0.1)' }}>BEFORE</div>
              </div>
              {/* Divider */}
              <div style={{ position: 'absolute', left: '50%', top: 0, bottom: 0, width: 2, background: 'linear-gradient(to bottom,transparent,rgba(79,124,255,0.9),transparent)', zIndex: 10, transform: 'translateX(-50%)' }}>
                <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,#4f7cff,#8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, boxShadow: '0 0 24px rgba(79,124,255,0.7)', fontWeight: 700 }}>✦</div>
              </div>
              <div style={{ position: 'relative', overflow: 'hidden', aspectRatio: '16/9' }}>
                ...
                <div style={{ position: 'absolute', top: 14, right: 14, background: 'rgba(79,124,255,0.2)', backdropFilter: 'blur(8px)', borderRadius: 8, padding: '5px 13px', fontSize: 12, fontWeight: 700, color: '#a0b4ff', border: '1px solid rgba(79,124,255,0.35)' }}>✦ AI REDESIGN</div>
              </div>
            </div>
            {/* Status bar */}
            <div style={{ background: '#0d1120', borderTop: '1px solid rgba(255,255,255,0.05)', padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
              ...
              <a href="/create" style={{ background: 'linear-gradient(135deg,#4f7cff,#8b5cf6)', border: 'none', color: 'white', fontSize: 12, fontWeight: 700, padding: '7px 18px', borderRadius: 8, cursor: 'pointer', textDecoration: 'none' }}>⬇ Download HD</a>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: '80px 28px' }}>
        <div style={{ textAlign: 'center', marginBottom: 56 }}>
          ...
          <h2 style={h2style}>Design in 3 simple steps</h2>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 20 }}>
          {[
            ...
            { num: '02', icon: '🎨', title: 'Pick a Design Style', desc: 'Choose from 8 curated styles. Add a custom description for extra precision.' },
            { num: '03', icon: '✨', title: 'Get AI Render', desc: 'Photorealistic redesign in under 10 seconds. Download in HD and share anywhere.' },
          ].map(s => (
            <div key={s.num} style={{ ...card, padding: '36px 30px', position: 'relative' }}
              onMouseOver={e => { const el = e.currentTarget as HTMLElement; el.style.transform = 'translateY(-4px)'; el.style.borderColor = 'rgba(255,255,255,0.18)'; el.style.boxShadow = '0 20px 50px rgba(0,0,0,0.3)' }}
              onMouseOut={e => { const el = e.currentTarget as HTMLElement; el.style.transform = 'translateY(0)'; el.style.borderColor = 'rgba(255,255,255,0.08)'; el.style.boxShadow = 'none' }}>
              ...
              <h3 style={{ fontSize: 20, fontWeight: 800, color: 'white', marginBottom: 10 }}>{s.title}</h3>
      ...
      {/* ── STYLE GRID WITH PHOTOS ── */}
      <section id="styles" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 28px 80px' }}>
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          <div style={label}>Design Styles</div>
          <h2 style={h2style}>8 stunning styles to explore</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14 }}>
          {[
            { label: 'Modern', img: IMGS.modern, color: '#4f7cff' },
            { label: 'Luxury', img: IMGS.luxury, color: '#fbbf24' },
            { label: 'Minimalist', img: IMGS.minimalist, color: '#e2e8f0' },
            { label: 'Scandinavian', img: IMGS.scandi, color: '#22c55e' },
            { label: 'Industrial', img: IMGS.industrial, color: '#f97316' },
            { label: 'Classic', img: IMGS.classic, color: '#d4a05a' },
            { label: 'Bohemian', img: IMGS.bohemian, color: '#ec4899' },
            { label: 'Japanese', img: IMGS.japanese, color: '#10b981' },
          ].map(s => (
            <div key={s.label} style={{ position: 'relative', borderRadius: 18, overflow: 'hidden', aspectRatio: '4/3', cursor: 'pointer', transition: 'transform 0.25s ease,box-shadow 0.25s ease' }}
              onMouseOver={e => { const el = e.currentTarget as HTMLElement; el.style.transform = 'translateY(-4px) scale(1.01)'; el.style.boxShadow = '0 20px 50px rgba(0,0,0,0.5)' }}
              onMouseOut={e => { const el = e.currentTarget as HTMLElement; el.style.transform = 'translateY(0) scale(1)'; el.style.boxShadow = 'none' }}>
              ...
              <div style={{ position: 'absolute', bottom: 12, left: 14 }}>
                <p style={{ fontSize: 14, fontWeight: 800, color: 'white' }}>{s.label}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section id="gallery" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 28px 80px' }}>
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          <div style={label}>Room Gallery</div>
          <h2 style={h2style}>Design any room in your home</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
          {/* Big card */}
          <div style={{ position: 'relative', borderRadius: 22, overflow: 'hidden', gridRow: 'span 2', cursor: 'pointer', transition: 'transform 0.25s' }}
            onMouseOver={e => (e.currentTarget as HTMLElement).style.transform = 'scale(1.01)'}
            onMouseOut={e => (e.currentTarget as HTMLElement).style.transform = 'scale(1)'}>
            <img src={IMGS.bedroom} alt="Bedroom" style={{ width: '100%', height: '100%', objectFit: 'cover', minHeight: 400 }} />
            ...
            <div style={{ position: 'absolute', bottom: 24, left: 24, right: 24 }}>
              ...
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>Modern Luxury style · Generated in 7.2s</p>
            </div>
            <div style={{ position: 'absolute', top: 16, right: 16, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(10px)', borderRadius: 10, padding: '6px 12px', fontSize: 12, color: 'rgba(255,255,255,0.7)', border: '1px solid rgba(255,255,255,0.1)' }}>✦ AI Render</div>
          </div>
          {/* Kitchen */}
          <div style={{ position: 'relative', borderRadius: 20, overflow: 'hidden', cursor: 'pointer', transition: 'transform 0.25s' }}
            onMouseOver={e => (e.currentTarget as HTMLElement).style.transform = 'scale(1.01)'}
            onMouseOut={e => (e.currentTarget as HTMLElement).style.transform = 'scale(1)'}>
            <img src={IMGS.kitchen} alt="Kitchen" style={{ width: '100%', height: '100%', objectFit: 'cover', minHeight: 192 }} />
          ...
          <div style={{ position: 'relative', borderRadius: 20, overflow: 'hidden', cursor: 'pointer', transition: 'transform 0.25s' }}
            onMouseOver={e => (e.currentTarget as HTMLElement).style.transform = 'scale(1.01)'}
            onMouseOut={e => (e.currentTarget as HTMLElement).style.transform = 'scale(1)'}>
            <img src={IMGS.bathroom} alt="Bathroom" style={{ width: '100%', height: '100%', objectFit: 'cover', minHeight: 192 }} />
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top,rgba(0,0,0,0.75) 0%,transparent 60%)' }} />
            <div style={{ position: 'absolute', bottom: 16, left: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div style={{ position: 'relative', borderRadius: 20, overflow: 'hidden', cursor: 'pointer', transition: 'transform 0.25s' }}
            onMouseOver={e => (e.currentTarget as HTMLElement).style.transform = 'scale(1.01)'}
            onMouseOut={e => (e.currentTarget as HTMLElement).style.transform = 'scale(1)'}>
            <img src={IMGS.office} alt="Office" style={{ width: '100%', height: 220, objectFit: 'cover' }} />
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top,rgba(0,0,0,0.75) 0%,transparent 60%)' }} />
            <div style={{ position: 'absolute', bottom: 16, left: 16 }}>
          ...
          <div style={{ borderRadius: 20, background: 'linear-gradient(135deg,rgba(79,124,255,0.15),rgba(139,92,246,0.12))', border: '1px solid rgba(79,124,255,0.2)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32, textAlign: 'center', height: 220 }}>
            ...
            <a href="/create" style={{ background: 'linear-gradient(135deg,#4f7cff,#8b5cf6)', border: 'none', color: 'white', fontSize: 13, fontWeight: 700, padding: '9px 22px', borderRadius: 10, cursor: 'pointer', textDecoration: 'none' }}>Explore All →</a>
          </div>
        </div>
      </section>
      ...
      <section id="features" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 28px 80px' }}>
        <div style={{ textAlign: 'center', marginBottom: 52 }}>
          ...
          <h2 style={h2style}>Everything you need to design</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(290px,1fr))', gap: 16 }}>
          {[
            ...
            { icon: '⬇️', title: 'HD Downloads', desc: 'Download renders at 1536×1024 resolution. Share with contractors or social media.' },
            { icon: '🔄', title: 'Unlimited Iterations', desc: 'Tweak the prompt and regenerate until it\'s perfect. Every time in seconds.' },
          ].map(f => (
            <div key={f.title} style={{ ...card, padding: '28px 26px' }}
              onMouseOver={e => { const el = e.currentTarget as HTMLElement; el.style.transform = 'translateY(-4px)'; el.style.borderColor = 'rgba(255,255,255,0.18)'; el.style.boxShadow = '0 20px 50px rgba(0,0,0,0.3)' }}
              onMouseOut={e => { const el = e.currentTarget as HTMLElement; el.style.transform = 'translateY(0)'; el.style.borderColor = 'rgba(255,255,255,0.08)'; el.style.boxShadow = 'none' }}>
      ...
      <section id="pricing" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 28px 80px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 20, alignItems: 'center' }}>
          {[
            { name: 'Free', price: '$0', features: ['3 AI renders/month', 'Basic design styles', 'Standard quality', '5 AI chat messages', 'Download renders'], cta: 'Get Started Free', highlight: false },
            { name: 'Pro', price: '$19', features: ['Unlimited AI renders', 'All 8 design styles', 'HD 1536px quality', 'Unlimited AI chat', 'Priority queue', 'Commercial rights'], cta: 'Start Pro', highlight: true },
            { name: 'Studio', price: '$49', features: ['Everything in Pro', '5 team members', 'White-label exports', 'API access', 'Priority support', 'Custom style presets'], cta: 'Start Studio', highlight: false },
          ].map(p => (
            <div key={p.name} style={{
              ...
              {p.highlight && <div style={{ position: 'absolute', top: 18, right: 18, background: 'linear-gradient(135deg,#4f7cff,#8b5cf6)', color: 'white', fontSize: 10, fontWeight: 800, padding: '4px 11px', borderRadius: 100, letterSpacing: '0.8px' }}>POPULAR</div>}
              <h3 style={{ fontSize: 24, fontWeight: 900, marginBottom: 4 }}>{p.name}</h3>
              <div style={{ marginBottom: 24 }}>
                <span style={{ fontSize: 52, fontWeight: 900, letterSpacing: '-2px' }}>{p.price}</span>
                <span style={{ fontSize: 15, color: p.highlight ? '#666' : 'rgba(255,255,255,0.4)' }}>/mo</span>
              </div>
              <ul style={{ listStyle: 'none', marginBottom: 28, flex: 1, display: 'flex', flexDirection: 'column', gap: 11 }}>
                {p.features.map(f => (
                  <li key={f} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 14, color: p.highlight ? '#444' : 'rgba(255,255,255,0.6)' }}>
                    <span style={{ color: p.highlight ? '#4f7cff' : '#4ade80', fontWeight: 800 }}>✓</span>{f}
                  </li>
                ))}
              </ul>
              <button style={{
                width: '100%', padding: 14, borderRadius: 14, fontWeight: 800, fontSize: 15, cursor: 'pointer',
      ...
      <section style={{ maxWidth: 950, margin: '0 auto', padding: '0 28px 100px' }}>
        <div style={{ position: 'relative', borderRadius: 32, overflow: 'hidden' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg,rgba(6,8,15,0.93) 0%,rgba(15,25,60,0.88) 50%,rgba(6,8,15,0.93) 100%)' }} />
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: 48 }}>
            ...
            <h2 style={{ fontSize: 'clamp(28px,4vw,52px)', fontWeight: 900, letterSpacing: '-1.5px', color: 'white', marginBottom: 14, lineHeight: 1.1 }}>Ready to redesign<br />your space?</h2>
            <p style={{ fontSize: 16, color: 'rgba(255,255,255,0.45)', marginBottom: 36, maxWidth: 420 }}>Join 10,000+ homeowners and designers using RoomGenie AI.</p>
            <a href="/create" style={{ ...btn(true), fontSize: 16, padding: '17px 44px', textDecoration: 'none' }}
              onMouseOver={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 16px 48px rgba(79,124,255,0.6)' }}
              onMouseOut={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 8px 28px rgba(79,124,255,0.4)' }}>
              ✦ &nbsp;Start Designing Free
            </a>
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.25)', marginTop: 16 }}>✓ No credit card &nbsp;·&nbsp; 3 free renders &nbsp;·&nbsp; 30 seconds to start</p>
          </div>
        </div>
      </section>
