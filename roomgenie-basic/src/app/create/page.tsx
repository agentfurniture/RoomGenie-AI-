    ...
    return (
      <div ref={resultRef} style={{ marginTop: 36, borderTop: '2px solid #e0e7ff', paddingTop: 36 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h2 style={{ fontSize: 28, fontWeight: 900, letterSpacing: '-0.8px', color: '#0f172a', marginBottom: 6 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 22, alignItems: 'start' }}>
          <div>
            ...
            {hasDims && result.floorPlan && (
              <div style={{ marginBottom: 18 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  ...
                  <span style={{ fontSize: 14, fontWeight: 700, color: '#0f172a' }}>2D Floor Plan</span>
                  <span style={{ fontSize: 12, color: '#94a3b8' }}>— furniture layout for your dimensions</span>
                </div>
                <div style={{ borderRadius: 16, overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,.08)', border: '1px solid #e8eaf0', position: 'relative', background: '#f8faff' }}>
                  <img src={result.floorPlan} alt="Floor plan"
                ...
                <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 6, paddingLeft: 4 }}>
                  ⓘ AI-generated layout based on {dims?.w}×{dims?.l}ft. Use as inspiration for furniture placement.
                </p>
              </div>
            )}

            {/* Prompt to add dimensions if missing */}
            {!hasDims && (
              <div style={{ background: '#f0f4ff', border: '1px solid #c7d2fe', borderRadius: 12, padding: '12px 16px', marginBottom: 16, display: 'flex', gap: 10, alignItems: 'center' }}>
                ...
                <p style={{ fontSize: 13, color: '#4f7cff', margin: 0 }}>
                  <strong>Want a floor plan?</strong> Go back to Step 1 and enter your room dimensions — the AI will generate both a 3D render AND a 2D floor plan.
                </p>
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 18, flexWrap: 'wrap' }}>
              ...
              <button onClick={() => { setResult(null); setStep(0); window.scrollTo({ top: 0, behavior: 'smooth' }) }} style={s.btnSecondary}>🔄 Restart</button>
            </div>
            ...
            {d?.description && (
              <div style={{ background: '#f8faff', border: '1px solid #e0e7ff', borderRadius: 14, padding: '18px 22px', marginBottom: 14 }}>
                ...
                {d?.spatialNote && typeof d.spatialNote === 'string' && d.spatialNote.length > 0 && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <button onClick={() => { setResult(null); setStep(0); window.scrollTo({ top: 0, behavior: 'smooth' }) }}
              style={{ ...s.btnPrimary, justifyContent: 'center', width: '100%' }}>✦ New Design</button>
            ...
            {!hasDims && (
              <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 12, padding: 14 }}>
                <p style={{ fontSize: 12, color: '#166534', lineHeight: 1.7, margin: 0 }}>
                  📐 <strong>Add dimensions</strong> in Step 1 next time to also get a 2D floor plan layout.
                </p>
              </div>
            )}
            ...
            <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 12, padding: 14 }}>
              <p style={{ fontSize: 12, color: '#92400e', lineHeight: 1.7, margin: 0 }}>
                💡 Upload your actual room photo in Step 1 for more accurate proportional results.
              </p>
            </div>
          </div>
        </div>
